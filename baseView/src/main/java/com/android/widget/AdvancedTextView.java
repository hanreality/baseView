...
public class AdvancedTextView extends AppCompatTextView {
    ...
    private int[] mUserPadding = new int[4]; // 用户设置的padding
    ...
    public AdvancedTextView(Context context, AttributeSet attrs) {
        this(context, attrs, R.style.TextAppearance_AppCompat);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mUserPadding[0] = getPaddingLeft();
        mUserPadding[1] = getPaddingTop();
        mUserPadding[2] = getPaddingRight();
        mUserPadding[3] = getPaddingBottom();
        ...
        getSingleLine(context, attrs);
    }

    private void getSingleLine(Context context, AttributeSet attrs) {
        int[] textAppearanceStyleArr = new int[0];
        int singleLineStyle = 0;
        try {
            ...
            field.setAccessible(true);
            singleLineStyle = (Integer) field.get(null);
        ...
        boolean singleLine = (boolean) a.getBoolean(singleLineStyle, false);
        a.recycle();
    ...
    public void setPadding(int left, int top, int right, int bottom, boolean updateUserPadding) {
    ...
    protected void onDraw(Canvas canvas) {
        if (mDrawableLeft != null || mDrawableRight != null) {
            int buttonContentWidth = getWidth() - getPaddingLeft() - getPaddingRight();
            float textWidth = 0f;
            final Layout layout = getLayout();
            if (layout != null) {
                for (int i = 0; i < layout.getLineCount(); i++) {
                    textWidth = Math.max(textWidth, layout.getLineRight(i) - layout.getLineLeft(i));
                }
            }
            ...
            if (gravity == Gravity.CENTER_HORIZONTAL) {
                if (translate > 1) {
                    setPadding(getPaddingLeft() + translate / 2, 0, getPaddingRight() + translate / 2, 0, false);
                }
            }
        } else {
            if (getPaddingLeft() != mUserPadding[0]
                    || getPaddingTop() != mUserPadding[1]
                    || getPaddingRight() != mUserPadding[2]
                    || getPaddingBottom() != mUserPadding[3]) {
